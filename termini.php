...
// Selected day within mesecni (calendar cell click)
$selDayMes = trim($_GET['dan_mes'] ?? '');
if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $selDayMes) || $selDayMes < $monthStart || $selDayMes > $monthEnd)
    $selDayMes = ($danas >= $monthStart && $danas <= $monthEnd) ? $danas : $monthStart;
...
if ($view === 'danas') {
    ...
    // Per-day counts for calendar grid (full month)
    $wh_mon = "WHERE $baseWh AND t.Datum BETWEEN '$monthStart' AND '$monthEnd' AND $inclFilter";
    if ($q !== '') $wh_mon .= searchClause($conn, $q, false);
    $dcQ = $conn->query("SELECT t.Datum, COUNT(*) AS cnt FROM termin t $joinK $wh_mon GROUP BY t.Datum");
    while ($row = $dcQ->fetch_assoc()) $mesDayCounts[$row['Datum']] = (int)$row['cnt'];
    $total = array_sum($mesDayCounts);
    // Cards for selected day
    $wh_day = "WHERE $baseWh AND t.Datum='$selDayMes' AND $inclFilter";
    ...
    <div class="pg-wrap">
        ...
        <div class="pg-toolbar ter-search-wrap">
            <form method="get" class="search-form" style="flex:1;min-width:0;">
                ...
                <?php if ($view === 'mesecni'): ?>
                    <input type="hidden" name="mesec" value="<?= htmlspecialchars($selMesec) ?>">
                    <input type="hidden" name="dan_mes" value="<?= htmlspecialchars($selDayMes) ?>">
                <?php endif; ?>
                <?php if ($view === 'svi'): ?>
        ...
        <div class="mes-cal">
            ...
            <div class="mes-cal-dow"><?= $dow_lbl ?></div>
            <?php endforeach; ?>
            <?php
            $firstDow = (int)(new DateTime($monthStart))->format('N');
            for ($i = 1; $i < $firstDow; $i++) echo '<div class="mes-cal-cell mes-cal-cell--other"></div>';
            $dayIter = new DateTime($monthStart);
            while (substr($dayIter->format('Y-m-d'), 0, 7) === $selMesec):
                $dk  = $dayIter->format('Y-m-d');
                $cnt = $mesDayCounts[$dk] ?? 0;
                $isActive  = $dk === $selDayMes;
                $isToday   = $dk === $danas;
                $href = 'termini.php?view=mesecni&mesec='.urlencode($selMesec).'&dan_mes='.urlencode($dk).$fFr.$fIncl.$qParam;
                $cls  = 'mes-cal-cell'.($cnt?' mes-cal-cell--has':' mes-cal-cell--empty').($isActive?' mes-cal-cell--active':'').($isToday?' mes-cal-cell--today':'');
            ?>
            <a class="<?= $cls ?>" href="<?= $cnt ? $href : '#' ?>">
                <span class="mes-cal-num"><?= $dayIter->format('j') ?></span>
                <?php if ($cnt): ?><span class="mes-cal-cnt"><?= $cnt ?></span><?php endif; ?>
            </a>
        ...
        <div class="today-section" style="margin-top:0.4rem;">
            ...
            <div class="ter-empty">
                <svg class="ter-empty-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.1" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                <h3 class="ter-empty-title"><?= $q!==''?'Nema rezultata':'Nema termina' ?></h3>
                <p class="ter-empty-sub"><?= $total>0?'Kliknite na dan u kalendaru da vidite termine.':'Nema termina u ovom mesecu.' ?></p>
            </div>
            <?php else: ?>
            <div class="today-cards">
            <?php foreach ($cardRows as $row):
