...
<?php
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['kontakt_submit'])) {
    ...
    if (empty($formErrors)) {
      ...
      $stmt = $conn->prepare("INSERT INTO poruka (Ime, Email, Telefon, Sadrzaj) VALUES (?, ?, ?, ?)");
  ...
  <div class="kp-body">
    ...
    <div class="kp-right">
      <div class="kp-form-header">
        <span class="ct-eyebrow">Pišite nam</span>
        <h2 class="kp-form-title">Pošaljite poruku</h2>
      </div>
      ...
      <?php if ($formStatus !== 'ok'): ?>
      <form class="kp-form" method="POST" action="kontakt.php" novalidate>
        <div class="ct-form-row2">
          ...
          <div class="ct-field">
            <label for="cf-email">Email <span class="ct-req">*</span></label>
        ...
        <div class="ct-field">
          ...
          <textarea id="cf-msg" name="sadrzaj" rows="6"
                    placeholder="Napišite nam…"><?= htmlspecialchars($_POST['sadrzaj'] ?? '') ?></textarea>
        </div>
        <button type="submit" name="kontakt_submit" class="ct-submit" style="align-self:flex-start;">
          Pošalji poruku
        </button>
      </form>
      <?php endif; ?>
